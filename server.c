...
void initialize_server(const char *server_name) {
    ...
    sem_t *sem_response = sem_open(sem_response_name, O_CREAT | O_EXCL, 0666, 0);

    if (sem_connect == SEM_FAILED || sem_command == SEM_FAILED || sem_response == SEM_FAILED) {
...
void handle_client_message(int client_id, const char *message, const char *server_name, GameData *game_data) {
    ...
    if (strncmp(message, "SEND_BOARD", 10) == 0) {
        ...
        if (sscanf(message + 7, "%d %d", &x, &y) == 2) {
            ...
            char response[BUFFER_SIZE];
            snprintf(response, sizeof(response), "ATTACK_RESULT %c", x, y, result);
            send_message_to_client(client_id, server_name, response);

            // Notify opponent about being attacked
            snprintf(response, sizeof(response), "OPPONENT_ATTACKED %c", result == 1 ? 'H' : 'M');
        ...
        if (receive_message(read_fd, buffer, BUFFER_SIZE) == 0) {
            ...
            if (strncmp(buffer, "CONNECT", 7) == 0) {
                ...
                if (sscanf(buffer, "CLIENT_%d:%s", &client_id, message) == 2) {
                    pthread_mutex_lock(&game_mutex);
                    if (strncmp(message, "PLACE", 5) == 0) {
                        int x, y, length;
                        char orientation;

                        if (sscanf(message, "PLACE %d %d %d %c", &x, &y, &length, &orientation) == 4) {
                            GameBoard *board = &game_data.board_players[client_id];
                            int result = place_ship(board, x, y, length, orientation);

                            char response[BUFFER_SIZE];
                            snprintf(response, sizeof(response), "PLACE_RESULT %d", result);
                            send_message_to_client(client_id, server_name, response);

                            // Signal that the client can proceed with the next command
                            sem_post(sem_response);
                            sem_post(sem_command);
                        }
                    } else if (strncmp(message, "SEND_BOARD", 10) == 0) {
                        ...
                        if (game_data.boards_ready[0] && game_data.boards_ready[1]) {
                            send_message_to_client(0, server_name, "ALL_BOARDS_READY");
                            send_message_to_client(1, server_name, "ALL_BOARDS_READY");
                            sem_post(sem_response); // Notify both players that they can start playing
                        }
                    } else if (strncmp(message, "ATTACK", 10) == 0)
